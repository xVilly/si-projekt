...
def loss_run(input,target,model,modifier,use_tanh,use_log,targeted,confidence,const):
  ...
  confidence = torch.tensor(confidence).type(torch.float64).cpu()
...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True,abort_early=True,
  ...
  for step in range(binary_search_steps):
    ...
    for iter in range(max_iter):
      ...
      var = var.view((-1,)+input.size()[1:]).cpu()
      ...
      real_modifier=torch.from_numpy(real_modifier_numpy).cpu()
...
if __name__=='__main__':
  ...
  inputs, targets = generate_data(test_loader,targeted,samples=2,start=0)
  timestart = time.time()
  ...
  # Fix: squeeze the singleton dimension for adv before passing to model
  adv_input = torch.from_numpy(adv).squeeze(1).cpu()  # shape: [N, 3, 32, 32]
  inputs_tensor = torch.from_numpy(inputs).cpu()

  if use_log:
    valid_class = np.argmax(F.softmax(model(inputs_tensor),-1).detach().cpu().numpy(),-1)
    adv_class = np.argmax(F.softmax(model(adv_input),-1).detach().cpu().numpy(),-1)
  else:
    valid_class = np.argmax(model(inputs_tensor).detach().cpu().numpy(),-1)
    adv_class = np.argmax(model(adv_input).detach().cpu().numpy(),-1)
  ...
  print("Total distortion: ", np.sum((adv.squeeze(1)-inputs)**2)**.5)
  ...
  for i in range(len(adv)):
    ...
    plt.imshow(((adv[i].squeeze(0)+0.5)).transpose(1,2,0))
  plt.tight_layout()
  if targeted:
    if solver=="newton":
